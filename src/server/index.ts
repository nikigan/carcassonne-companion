...
import { HOST_ONLY_ACTIONS } from '../game/protocol'
...
interface Persisted {
  ...
  // The creator's host token, recorded at seed time. When set, only action
  // messages carrying a matching token may run the host-only actions. Unset
  // (un-hosted) rooms leave those actions open to everyone.
  hostToken?: string
}
...
export class GameRoom extends DurableObject<Env> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (request.method === 'POST') {
      const body = (await request.json()) as { state?: unknown; hostToken?: unknown }
      ...
      if (empty) {
        ...
        this.data = { state, seq: 0, recentActionIds: [], hostToken }
        this.persist(); this.bumpIdleAlarm()
      }
  ...
  async webSocketMessage(ws: WebSocket, raw: string | ArrayBuffer) {
    ...
    // Host-only gating: when a room has a recorded host token, the two nuclear
    // actions require a matching token on the message. Un-hosted rooms (no
    // recorded token) leave them open, for backward-compat/safety.
    if (
      HOST_ONLY_ACTIONS.includes(msg.action.type) &&
      this.data.hostToken &&
      msg.hostToken !== this.data.hostToken
    ) {
      ws.send(JSON.stringify({ type: 'error', message: 'host only' } satisfies ServerMessage))
      return
    }
    try {
